...
--do ZGV:Print("Zygor Talent Advisor is temporarily disabled. We apologize for the inconvenience.") ZTA=nil ZygorTalentAdvisor=nil return end -- ZTA is taken down for repairs.


tinsert(ZGV.startups,{"ZTA",function(self)
	ZTA:Startup()
end})
...
function ZTA:Startup()
	...
	-- LEGION TEMP FIX, button no longer exists?
	...
	for i=1, #StaticPopup_DisplayedFrames do --Loop through all shown static popups
		...
		if popup.which=="CONFIRM_LEARN_TALENTS" then --our popup.
			...
			for i,talentid in ipairs(talents) do
				local _,_,tex=GetTalentInfo(talentid)
				local row = i
				--local column = 

				if ZTA.currentBuild.build[row]~=talentid then
					local _,_,sugtex=GetTalentInfo(ZTA.currentBuild.build[row])

					local sugtalent = "|T"..sugtex..":0:0:0:0|t |cffffdd00"..GetTalentLink(ZTA.currentBuild.build[row])
					local talent = "|T"..tex..":0:0:0:0|t |cffffdd00"..GetTalentLink(talentid)
...
function ZTA:SetCurrentBuild(text)
	if not ZGV.db.profile.talenton then return end
	ZTA.BuildSelect.Dropdown:SetValue(text)-- talentpopup_build
...
function ZTA:GetUserBuild()
	...
	for tier=1, MAX_TALENT_TIERS do
		for column=1, NUM_TALENT_COLUMNS do
			local id,n,_,selected,available=GetTalentInfo(tier, column, GetActiveSpecGroup())
			if selected then
				learnbuild[tier]= (tier-1)*NUM_TALENT_COLUMNS+column
...
function ZTA:CompareBuild()	
	...
	for i=1, #ZTA.currentBuild.build do
		...
		--print("Tier",i,"Suggested",sugindex,"Current",userindex)
	...
	if needTalents then 
		ZTA.talentstatus="learn"
	elseif not talentsMatch then 
		ZTA.talentstatus="wrong"
	elseif talentsMatch then 
		ZTA.talentstatus="correct"
	end

	ZTA.needToLearn = learnTalents --set these into a table where we can get them when we want to learn them

	if not talentsMatch and #wrongTalents==GetMaxTalentTier() then 
		ZTA:SetSuggestion(L['talentpopup_specred'],145) --all of their talents are wrong.
	elseif #learnTalents >0 or #wrongTalents >0 then 
		ZTA:LearnSuggestion(learnTalents,wrongTalents) --They either can select talents or we need to take out the wrong ones.
	elseif talentsMatch then 
		ZTA:SetSuggestion(L['talentpopup_green'],125) --They followed our build perfect
	end
...
function ZTA:GetSuggestionFormatted(learnTalents)
	...
	--Spoo(nil, nil, learnTalents)
	for i,talentindex in ipairs(learnTalents) do
		--print("index",index)
		local tier = ceil(talentindex/3)
		local column = (talentindex-1)%3+1
		local id,name,tex=GetTalentInfo(tier,column,GetActiveSpecGroup())

		if not id then
			id,name,tex = -1,"Unknown talent",""
			ZGV:Debug("Unable to get talent details for "..talentindex)
		end

		--local _,_,tex=GetTalentInfo(index)

		tinsert(sugformatted,{tex=tex,index=talentindex,id=id})
	end
	return sugformatted
end
...
--[[function ZTA:GetSuggestionFormatted(learnTalents)
	...
	for i,talentindex in ipairs(learnTalents) do
		...
		local tex
		if ZGV.Expansion_Warlords then
			local tier = ceil(talentindex/3)
			local column = talentindex%3+1
			_,_,tex=GetTalentInfo(tier,column,GetActiveSpecGroup())
		else
			_,tex=GetTalentInfo(index)
		end
...
function ZTA:LearnSuggestion(learnTalents,wrongTalents)
	if not learnTalents then learnTalents = {} end
	if not wrongTalents then wrongTalents = {} end
	...
	-- TODO: Update for WoD.
	--if ZGV.Expansion_Warlords then return end
	...
	if #wrongTalents>0 then
		...
		for tier,data in ipairs(sugformatted) do
			talent = "|T"..data.tex..":0:0:0:0|t |cffffdd00"..GetTalentLink(data.id)
			s=s.."                "..talent.."\n"
		end
...
function ZTA:PreviewSuggestedTalents(talents)
	...
	for tier,index in ipairs(talents) do
		local row = ceil(index/3)
		local column = (index-1)%3+1
		local talentid = GetTalentInfo(row,column,spec)
		--print(index,"=",row,column,talentid)
		local prefix = "PlayerTalentFrameTalents"
		local tier=_G[prefix]["tier"..row]

		--[[
		if GetTalentRowSelectionInfo(row) and tier.selectionId~=talentid then
			--If they have not selected a talent for that row, and nothing is selected.
			PlayerTalentFrame_SelectTalent(row,talentid) --This is causing a addon conflict at the moment.
		end
		--]]
	end
	--TalentFrame_Update(PlayerTalentFrameTalents) -- Blizzard function!  -- 2014-10-15 17:18:46 - this is now called by PlayerTalentFrame_SelectTalent automatically.
end
...
function ZTA:ResetAllTalentIcons()
--Spoo(nil, nil, ZTA)
	if not ZTA.TalentIcons then return end
	--[[
	if GetNumTalents then -- Mists of Pandaria
	...
	else -- Warlods of Draenor
	--]]
		for i=1,21 do
			if ZTA.TalentIcons[i] then
				if ZTA.TalentIcons[i].cross then
					ZTA.TalentIcons[i].cross=nil
					ZTA.TalentIcons[i].tex:SetTexture(ZGV.DIR.."\\ZygorTalentAdvisor\\Skin\\popout-button")
					ZTA.TalentIcons[i].tex:SetTexCoord(0, 1, 0, 1) --Reset whatever the old position was.
...
function ZTA_SelectSpec_Flash(self,elapsed)
	...
	--if #ZTA.needToLearn > 0 and ZGV.db.profile.talentpopup==3 then ZTA:AutoLearn() end  -- 2014-10-15 20:42:48 - autolearn removed. ~sinus
