...
local MAJOR, MINOR = "HereBeDragons-ZGV", 27
...
--[[
	if id==1077 then
		if mapData[id].name=="Val'sharah" then print("WTF, this is Dreamgrove, not Val!") else print("Dreamgrove named OK") end
	        local originalInstanceID, _, _, left, right, top, bottom = GetAreaMapInfo(id)
	end
--]]
    ...
    function HereBeDragons:GatherMapData()
        gatherMapData()
	--if HereBeDragons.test_dreamgrove then print("after init:") HereBeDragons:test_dreamgrove() end
    end
...
local function OnEvent(frame, event, ...)
	--print("HBD: "..event)
    if event=="PLAYER_ENTERING_WORLD" and not HereBeDragons.gathered then
    	HereBeDragons:GatherMapData()
    	HereBeDragons.gathered=true
    end
    UpdateCurrentPosition()
	--print("HBD:GPZ says",HereBeDragons:GetPlayerZone())
	--print("  TSL ".. ((abs((ZGV.HBD:TranslateZoneCoordinates(0.5,0.5,1072,0,1024,0) or 0) - 0.347)<0.1) and "OK" or "FAIL"))
end
...
function HereBeDragons:GetPlayerZonePosition(allowOutOfBounds)
    ...
    if x and y then
	x, y = self:GetZoneCoordinatesFromWorld((x or 0), (y or 0), currentPlayerZoneMapID, currentPlayerLevel, allowOutOfBounds)
        return x, y, currentPlayerZoneMapID, currentPlayerLevel, currentMapFile, currentMapIsMicroDungeon
    else
        return nil, nil, currentPlayerZoneMapID, currentPlayerLevel, currentMapFile, currentMapIsMicroDungeon
    end
    return nil, nil, nil, nil
end

--[[
local function test_dreamgrove() -- self-test: The Dreamgrove
        local originalInstanceID, _, _, left, right, top, bottom = GetAreaMapInfo(1077)
	local x,y = HereBeDragons:TranslateZoneCoordinates(0.5,0.5,1077,0,1018,0)   if not x or not y or abs(x-0.410)>0.1 or abs(y-0.267)>0.1 then print("The Dreamgrove / Val'Sharah cross-coords are WRONG.") else print("The Dreamgrove / Val'Sharah coords are OK!") end
end
HereBeDragons.test_dreamgrove = test_dreamgrove
--/run ZGV.HBD.test_dreamgrove()
--/run ZGV.HBD:GatherMapData()
test_dreamgrove()
--]]
