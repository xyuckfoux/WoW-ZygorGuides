...
function MapSpot:Show()
	if not self.waypoint then
		self.waypoint = ZGV.Pointer:SetWaypoint(nil,self.map,self.x,self.y,{title=self:GetTitle(),type="poi",icon=ZGV.DIR .. "\\Skins\\minimaparrow-gold-dot",edgeicon=ZGV.DIR .. "\\Skins\\minimaparrow-gold-edge",overworld=false,onminimap="zonedistance"})
...
local function MapSpot_ObjectToString(obj)
	if obj.type=="item" and not obj.hidden then
		...
		local data = id and ZGV:GetItemInfo(id) or ""
		local perc = ""
